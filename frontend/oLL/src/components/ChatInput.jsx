...
function ChatInput({getToken,chatId,historyHandler,currentResposne,startGenerating,setstartGenerating,setisloading,SuggestionArray,fillSugg,suggestion,Headline,submitHandler}) {
  ...
  <div className="  shadow-md  rounded-[45px] bg-[#202020] backdrop-blur-md border-2  shadow-[#000000] transition-all duration-300  border-[#404040] min-h-[7.5vh] flex flex-col justify-between   md:w-[50vw] w-[90%] overflow-hidden max-h-[30vh] md:max-h-[30vh]">
    ...
    <button style={{display:startGenerating?"block":"none"}} onClick={async ()=>{await historyHandler(currentResposne,await getToken(),chatId),setstartGenerating(false),setisloading(false)}} className=" hover:opacity-50 hover:cursor-pointer transition-all duration-100 bg-white h-[35px] flex p-2 justify-center mb-1.5 md:mb-2.5 w-[35px] rounded-full"><img src={stop} /></button>
    </div>
  </div>
</div>
